 ...
 * (`src/app.css:24`, applied on `body` at `src/app.css:239`), so the existing
 * `grid-auto-rows: 6rem` (`WidgetGrid.svelte:74`) is 6 × 14 = 84px and the
 * `--space-4` gap (`src/app.css:37`, `1rem`) is 14px. The breakpoints are the
...
/** Desktop grid columns. The free-form layout targets this 4-column mode only. */
export const GRID_COLUMNS = 4;

/**
 * One widget height unit is one 6rem grid row, i.e. 1 row per unit. The
...
/** Grid row height in rem, `grid-auto-rows` (`WidgetGrid.svelte:74`). */
export const GRID_ROW_HEIGHT_REM = 6;

/** Grid row height in px: 6rem × 14px root = 84px. */
...
/** Desktop minimum width: ≥ 64rem (896px) renders the 4-column grid. */
