...
/**
 * Wrapper for the turn-permission suite (task #378).
 *
 ...
 * turn-assembler + permission-store suite runs in an isolated child `bun test`
 * process. The suite file is `turn-permission.suite.ts` (no `.test` suffix) so
 * the parent glob does not pick it up.
 *
...
test('turn-permission suite passes in an isolated child process', async () => {
	const proc = Bun.spawn(['bun', 'test', './src/lib/server/turn-permission.suite.ts'], {
		cwd: repoRoot,
		env: process.env,
		stdout: 'pipe',
		stderr: 'pipe'
